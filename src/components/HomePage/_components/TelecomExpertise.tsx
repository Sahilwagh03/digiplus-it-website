        ...
        <div className="text-center font-primary mb-16 max-w-3xl mx-auto">
          <h2 className="font-primary text-3xl md:text-4xl lg:text-5xl font-bold flex justify-center flex-wrap gap-x-2">
            {title.map((part, index) => (
              <span
                key={index}
                className={`${part.highlight ? "text-primary-red" : ""}`}
              >
                {part.text}
              </span>
            ))}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature, index) => {
            ...
            return (
              <div
                key={index}
                className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm hover:shadow-xl transform hover:-translate-y-2 transition duration-300 ease-in-out group"
              >
