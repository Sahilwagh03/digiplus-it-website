...
const CentralIcon = ({ icon, alt, children = null, width = 64, height = 64 }) => (
...
const CollabIcon = ({ icon, width, height, index, alt, onClick }) => (
  <li
    onClick={onClick}
    className="cursor-pointer absolute top-0 left-1/2 h-1/2 -ml-[2rem] origin-bottom"
    style={{ transform: `rotate(${index * 45}deg)` }}
  >
    <div
      className="relative -top-[2rem] flex w-[4rem] h-[4rem] rounded-xl bg-white/80 transition-all duration-300 hover:scale-125 hover:shadow-md"
      style={{ transform: `rotate(${-index * 45}deg)` }}
    >
...
const CollabIconContainer = ({ children }) => <ul>{children}</ul>;
